...
`list_templates`-tool (döpt om från `list_pro_templates` 2026-07-20, se `DECISIONS.md`)/`GET /api/v1/pro-templates` anropar RPC:n `get_pro_templates_for_mcp_key` (beviljad direkt till `anon` i `promptbanken`-repot — nyckelhashen är i sig beviset på behörighet, samma modell som `verify_mcp_key`). Kräver bara `SUPABASE_URL`/`SUPABASE_ANON_KEY`, ingen `mcp_server`-roll/JWT. Sedan 2026-07-19 (öppen katalog, se `DECISIONS.md`) returnerar RPC:n alltid alla mallar med full `prompt_text` och `is_unlocked=true` — ingen teaser, ingen nyckel/plan krävs. RPC-namnet, REST-pathen och `pro_templates.py`-modulnamnet är historiska och avsiktligt oförändrade — bara MCP-verktygsnamnet döptes om.
