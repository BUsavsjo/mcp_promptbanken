...
## Klart
- [x] Lade till `workspace_status`-fält i `list_skills_simple` och REST `GET /api/v1/skills` — visar `"invalid_key"` när en `X-MCP-Key`/`Authorization`-nyckel skickas men inte matchar ett aktivt workspace (täcker återkallad, felstavad eller inaktiverad nyckel), utan att blockera de publika skillsen. Fältet utelämnas helt om ingen nyckel skickas. Se README för detaljer och känd begränsning (RPC:n skiljer inte ut orsak ännu).
